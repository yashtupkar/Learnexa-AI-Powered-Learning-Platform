...
const QuizInterface = () => {
  ...
  useEffect(() => {
    if (mode === "practice") {
      ...
      // If we're in practice mode and there's an answer for current question, show explanation
      if (answers[currentQuestion] !== undefined) {
        setShowExplanation(true);
      }
    }
  }, [mode, currentQuestion, answers]);
  ...
  const updateUrlFromState = () => {
    ...
    navigate(`/quiz/${quizId}?tab=${newTab}&mode=${mode}`, { replace: true });
  };
  ...
  useEffect(() => {
    ...
    } else if (timeLeft === 0 && quizStarted && quizData && !isSubmitting) {
      handleSubmit();
    }
  }, [timeLeft, quizStarted, showResults, quizData, isSubmitting]);

  // Check if current question is answered and handle explanation display
  useEffect(() => {
    const isAnswered = answers[currentQuestion] !== undefined;
    setAnswered(isAnswered);

    // Show explanation in practice mode when question is answered
    if (quizMode === "practice" && isAnswered) {
      // Small delay to allow answer animation to complete
      setTimeout(() => {
        setShowExplanation(true);
      }, 300);
    } else if (quizMode !== "practice") {
      // Only hide explanation if not in practice mode
  ...
  const nextQuestion = () => {
    if (quizData && currentQuestion < quizData.questions.length - 1) {
      ...
      navigate(`/quiz/${quizId}?tab=question${newQuestion + 1}&mode=${mode}`, {
        replace: true,
      });
    }
  };
  ...
  const prevQuestion = () => {
    if (currentQuestion > 0) {
      ...
      // In practice mode, keep explanation shown if previous question is already answered
      if (quizMode === "practice" && answers[newQuestion] !== undefined) {
        setShowExplanation(true);
      } else if (quizMode !== "practice") {
        setShowExplanation(false);
      }
  ...
  const handleStartQuiz = (mode) => {
    setQuizMode(mode);
    setQuizStarted(true);
    navigate(`/quiz/${quizId}?tab=question1&mode=${mode}`, { replace: true });
  };

  const calculateDetailedScore = () => {
    // Handle case where quiz data isn't loaded
    ...
    // Map through each question to create detailed results
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${backend_URL}/api/quiz/submit-quiz`,
        {
          quizId: quizId,
          userId: user._id,
          score: scoreDetails,
  ...
  const resetQuiz = (updateUrl = true) => {
    ...
    setIsSubmitting(false);
    if (updateUrl) {
      navigate(`/quiz/${quizId}?tab=startscreen&mode=${mode}`, {
        replace: true,
      });
    }
  };
  ...
  if (!quizStarted && quizData) {
    return (
      <QuizStartScreen
        quizData={quizData}
        setQuizStarted={handleStartQuiz}
      />
    );
  }
  ...
  return (
    <div
      className={`min-h-screen w-full bg-white scrollbar-custom dark:bg-zinc-900 `}
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <button
              ...
              className="p-1.5 rounded-lg text-white hover:bg-gray-200/20 cursor-pointer  transition-colors"
            >
      ...
      <div className="flex h-screen overflow-hidden pb-20 md:pt-0 md:pb-0">
        ...
        <AnimatePresence>
          ...
          <motion.div
            ...
            className="h-full w-80 bg-white/95 dark:bg-zinc-900 backdrop-blur-xl shadow-2xl border-r border-gray-200 dark:border-zinc-600 fixed z-50 md:z-40"
            style={{
              width: "85vw",
              maxWidth: "320px",
            }}
            ...
            <div className="p-4 md:p-6 h-full flex flex-col overflow-y-auto">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 gap-2 md:gap-3 mb-6 p-2 md:p-3">
                  {quizData.questions.map((_, index) => (
                    <motion.button
                      ...
                      className={`relative p-2 md:p-3 rounded-lg md:rounded-xl flex flex-col items-center justify-center transition-all duration-300 shadow-sm ${quizMode === "practice"
              ...
              <div className="space-y-3 md:space-y-4 pt-4 md:pt-6 border-t border-gray-200 dark:border-zinc-700">
                ...
                <motion.button
                  ...
                  className={`w-full py-2 md:py-3 cursor-pointer rounded-lg md:rounded-xl font-semibold shadow-lg transition-all duration-300 ${isSubmitting
                      ? "bg-gray-400 cursor-not-allowed"
                      : "bg-gradient-to-br from-indigo-600 to-indigo-400 text-white  hover:from-indigo-600 hover:to-purple-700"
                    }`}
                >
                ...
                <motion.button
                  ...
                  className="w-full bg-gradient-to-r from-red-400 cursor-pointer to-red-600 text-white py-2 md:py-3 rounded-lg md:rounded-xl font-semibold shadow-lg shadow-red-500/25 hover:from-red-600 hover:to-red-700 transition-all duration-300"
                >
                  <div className="flex items-center justify-center space-x-2">
              ...
              <div className="mt-3 flex flex-wrap gap-1">
                {quizData.questions.map((_, index) => (
                  <button
                    ...
                    onClick={() => {
                      setCurrentQuestion(index);
                      // Only show explanation if in practice mode AND an answer exists
                      if (mode === "practice" && answers[index] !== undefined) {
                        setShowExplanation(true);
                      } else {
                        setShowExplanation(false);
                      }
                    }}
                    className={`w-6 h-6 md:w-8 md:h-8 rounded-md text-xs flex items-center justify-center transition-all ${index === currentQuestion
        ...
        <div className="fixed bottom-0 left-0 right-0 z-20 bg-white/95 dark:bg-black backdrop-blur-lg border-t border-gray-200 dark:border-zinc-600 p-3 shadow-sm">
          <div className="flex justify-between items-center">
            <button
              ...
              className={`px-3 py-2 flex gap-2 items-center rounded-lg ${currentQuestion === 0
              ...
              {currentQuestion < quizData.questions.length - 1 ? (
                <>
                  ...
                  <button
                    onClick={nextQuestion}
                    disabled={!answered}
                    className={`px-3 py-2 flex gap-2 items-center rounded-lg ${!answered
                        ? "bg-gray-200 text-gray-500 dark:bg-gray-700 dark:text-gray-500 cursor-not-allowed"
                ...
                <button
                  onClick={handleSubmit}
                  disabled={isSubmitting}
                  className={`px-3 py-2 rounded-lg ${isSubmitting
                      ? "bg-gray-400 cursor-not-allowed text-white"
                      : "bg-gradient-to-br from-indigo-600 to-purple-400 text-white cursor-pointer hover:from-blue-600 hover:to-purple-600"
                    }`}
                >
      ...
      {showConfetti && (
        <div className="fixed inset-0 pointer-events-none z-50">
          {/* Add confetti animation here if needed */}
        </div>
      )}
    </div>
  );
