...
const CountdownScreen = ({ onComplete, theme }) => {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(timer);
  }, [onComplete, currentStep, audioContext, steps]);
...
const QuizStartScreen = ({
  ...
  const updateURL = (mode) => {
    ...
    newParams.set("tab", "startscreen");
    newParams.set("mode", mode);

    navigate(`/quiz/${quizId}?${newParams.toString()}`, {
      replace: true,
    });
  };
 ...
 const handleStartPracticeQuiz = () => {
  ...
  const handleCountdownComplete = () => {
    ...
    setQuizStarted(true);
     const newParams = new URLSearchParams(searchParams);
     newParams.delete("tab"); // Remove the startscreen tab
     navigate(`/quiz/${quizId}?${newParams.toString()}`, { replace: true });
  };
 ...
 return (
   <>
     ...
     >
       {/* Interactive Parallax Background */}
       <motion.div className="absolute inset-0" animate={controls}>
         {/* Animated grid lines - Reduce number on mobile */}
         <div className="absolute inset-0 opacity-10">
           {Array.from({ length: window.innerWidth < 768 ? 10 : 20 }).map(
             (_, i) => (
               <motion.div
                 key={`line-${i}`}
                 className="absolute top-0 bottom-0 w-px bg-white"
                 style={{ left: `${i * (window.innerWidth < 768 ? 10 : 5)}%` }}
                 animate={{
                   opacity: [0.3, 0.7, 0.3],
                 }}
                 transition={{
         ...
         {particles
           .slice(0, window.innerWidth < 768 ? 15 : particles.length)
           .map((particle) => (
             <motion.div
               key={particle.id}
               className="absolute rounded-full bg-white/20 backdrop-blur-sm"
         ...
         <motion.div
           className="absolute top-1/4 left-1/4 w-32 h-32 md:w-64 md:h-64 rounded-full blur-3xl bg-gradient-to-r from-cyan-500/20 to-fuchsia-500/20"
           animate={{
             scale: [1, 1.2, 1],
             opacity: [0.3, 0.6, 0.3],
           }}
         ...
         />
         <motion.div
           className="absolute bottom-1/4 right-1/4 w-36 h-36 md:w-72 md:h-72 rounded-full blur-3xl bg-gradient-to-r from-purple-500/20 to-blue-500/20"
           animate={{
             scale: [1.2, 1, 1.2],
             opacity: [0.4, 0.7, 0.4],
           }}
       ...
       <div className="relative z-10 p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center min-h-screen w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto">
         {/* Header with User Stats */}
         <motion.div
           initial={{ opacity: 0, y: -30 }}
           animate={{ opacity: 1, y: 0 }}
         ...
         >
           <div className="flex items-center gap-2 sm:gap-3">
             ...
             <div>
               ...
               <div className="text-[10px] sm:text-xs text-gray-300/80">
                 {quizData.grade} Level
               </div>
             </div>
           </div>
         ...
         <AnimatePresence>
           {showSettings && (
             <motion.div
               ...
               animate={{ opacity: 1, height: "auto" }}
               exit={{ opacity: 0, height: 0 }}
               className="w-full bg-gray-900/80 backdrop-blur-xl rounded-xl p-3 sm:p-4 mb-4 sm:mb-6 border border-white/10 overflow-hidden"
             >
               <div className="space-y-3 sm:space-y-4">
                 ...
                 <div>
                   <span className="text-xs sm:text-sm text-white mb-1 sm:mb-2 block">
                     Theme
                   </span>
         ...
         <motion.div
           ...
           animate={{ opacity: 1, y: 0 }}
           transition={{ duration: 0.5, delay: 0.1 }}
           className="w-full bg-gray-900/60 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 mb-4 sm:mb-6 border border-white/10 relative overflow-hidden"
         >
           <div className="absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-gradient-to-bl from-white/10 to-transparent rounded-full -translate-y-10 sm:-translate-y-16 translate-x-10 sm:translate-x-16" />

           <div className="relative">
             <div className="flex items-center justify-between mb-3 sm:mb-4">
               <div className="flex items-center gap-1 sm:gap-2">
                 <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-r from-orange-500 to-red-500 rounded-lg flex items-center justify-center shadow-md">
                   <Zap className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
                 </div>
             ...
             <motion.h1
               className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-3 sm:mb-4 leading-tight"
               whileHover={{ x: 2 }}
             >
               {quizData.quiz_title}
             </motion.h1>
             ...
             <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4 mb-3 sm:mb-4">
               <motion.div
                 ...
                 whileHover={{ y: -2 }}
               >
                 <div className="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-1 sm:mb-2 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg flex items-center justify-center">
                   <Target className="w-2 h-2 sm:w-3 sm:h-3 md:w-4 md:h-4 text-white" />
                 </div>
               ...
               <motion.div
                 ...
                 whileHover={{ y: -2 }}
               >
                 <div className="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-1 sm:mb-2 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg flex items-center justify-center">
                   <Timer className="w-2 h-2 sm:w-3 sm:h-3 md:w-4 md:h-4 text-white" />
                 </div>
                 <div className="text-[10px] sm:text-xs text-gray-300">
                   Duration
                 </div>
         ...
         >
           ...
           <div className="grid grid-cols-2 gap-2 sm:gap-3">
             ...
             <motion.button
               ...
               whileTap={{ scale: 0.98 }}
             >
               <Book className="w-3 h-3 sm:w-4 sm:w-4" />
         ...
         >
           ...
           <motion.button
             className="flex items-center gap-1 bg-white/10 backdrop-blur-md px-2 py-1 sm:px-3 sm:py-1.5 rounded-full border border-white/20 hover:bg-white/20 transition-all"
             whileHover={{ scale: 1.05 }}
             whileTap={{ scale: 0.95 }}
           >
             <Share2 className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
